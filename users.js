...
router.get('/member', (req, res, next) => {
  res.sendFile(path.join(__dirname, 'public', 'MemberPage.html'));
});
// Middleware to set session data
router.get('/login', (req, res, next) => {
  req.session.username = 'jojo'; // Example of setting session data
...
router.post('/xapi/insertCategories', async (req, res, next) => {const MongoClient = require('mongodb').MongoClient;
//app.post('/api/insertCategories', async (req, res) => {
//try {
  await client.connect();
  const db = client.db("FashionStore");
  console.log('/api/insertCategories-req.body:',req.body);
...
router.put('/xapi/updateCategories/', async (req, res, next) => {const MongoClient = require('mongodb').MongoClient;
...
router.delete('/xapi/deleteCategories/', async (req, res, next) => {const MongoClient = require('mongodb').MongoClient;
try {
await client.connect();
    const db = client.db("FashionStore");
...
router.get('/api/users/id/:id', async (req, res, next) => {const MongoClient = require('mongodb').MongoClient;
  try {
    ...
    try {
      ...
      console.log("user.js - /api/user/id/" + id, user);
      res.json(user);
    } catch (error) {
